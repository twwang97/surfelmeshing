...
int LIBVIS_MAIN(int argc, char** argv) {
  ...
  // Load dataset.
  RGBD_INPUT_CLASS RGBD_INPUT_DATA(argparser.data.dataset_folder_path, argparser.data.trajectory_filename, argparser.data.invert_quaternions); 
  ...
  const usize RGBD_TOTAL_FRAMES = RGBD_INPUT_DATA.data_size_; 
  ...
  surfel2cpu_task surfel2cpu(RGBD_TOTAL_FRAMES, argparser);
  ...
  for (usize frame_index = argparser.data.start_frame; frame_index < RGBD_TOTAL_FRAMES && !quit; ++ frame_index) {
    ...
    // ### Input data loading ###
    RGBD_INPUT_DATA.Read1RGBDDataset(frame_index, &rgbd_video); 
    rgbd_video.depth_frame_mutable(frame_index)->GetImage();
    rgbd_video.color_frame_mutable(frame_index)->GetImage();
    
    if (frame_index == argparser.data.start_frame){
      // Set the 1st view
      mainViewers.setMeshFirstWindowDirection(
                    rgbd_video.depth_frame_mutable(0)->frame_T_global().rotationMatrix().transpose() * Vec3f(0, -1, 0), 
                    rgbd_video.depth_frame_mutable(0)->global_T_frame() * Vec3f(0, 0, 1));
      surfel_meshing.setWindow(mainViewers.render_window); 
      surfel2cpu.setWindow(&surfel_meshing, mainViewers.render_window); 
    }
    ...
    ///////////////////
    // if (frame_index % 2 == 0) continue; ///////////
    // if (frame_index > 2000) is_last_frame = true; ///////////
